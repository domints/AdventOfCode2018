...
namespace Day3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<int, List<string>> fabricPieces = new Dictionary<int, List<string>>();
            foreach(var item in items) ClaimFabric(item, claimsDict, fabricPieces);

            Console.WriteLine(fabricPieces.First(kvp => kvp.Value.All(pid => claimsDict[pid] == 1)).Key);
        }
